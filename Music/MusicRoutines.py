...
def FreqCube(cube, n, runtime):
    ...
    print(chunks)
    # Create an instance of CubeAudio named c
    c = CubeAudio(chunksize=CHUNK, rate=RATE, trim=TRIM)
    ...
    # break up the area between the minimum and maximum averages into h chunks
    ...
    for t in range(runtime):
        ...
        # read an array of fourier transform data and take the log base 10 of it
        farray = np.log10(c.readfourier())

        # add the line to the data to compensate for lower amplitudes at high frequency
        farray = farray + detrend

        # xstep is the size of a subarray.
        xstep = int(len(farray) / len(chunks))

        chunkz = []  # array into which we will store the z values

        for i in range(len(chunks)):

            # create a subarray of length xstep that starts at xstep*i
            subarray = farray[i * xstep:(i + 1) * xstep]
            avg = np.mean(subarray)  # find the average value of the subarray

            # find the z value. Will be = height when avg = maxavg and 0 when avg = minavg
            z = int(((avg - minavg) * vdiv) / (maxavg - minavg))
            if (z < 0):
                z = 0
            elif (z > vdiv):
                z = vdiv
        ...
        for i in range(len(chunks)):
            # light up our collumns defined by the chunks array to heights stored in chunkz
            cube.LightColumn(chunks[i], chunkz[i])
...
def Graphsim(cube):
    ...
    # how many low frequency points we want to cut off (tend to have very large noise for unknown reason)
    TRIM = 20
    ...
    def animate(i):
        ...
        # read an array of fourier data and take the log base 10 of it
        farray = np.log10(c.readfourier())
        ...
        # xstep is the size of a subarray. We will make
        xstep = int(len(farray) / len(chunks))
